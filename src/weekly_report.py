...
async def analyze_all_tracks_with_llm(tracks_list):
    ...
    # 准备上下文 (丰富仓库信息：Star数、主导语言、主题)
    context_lines = []
    for track in tracks_list:
        context_lines.append(f"### 赛道 ID: {track['id']} (规模: {track['count']} 个项目关联)")
        for r in track['repos'][:15]:
            lang_str = f" [语言: {r.get('language')}]" if r.get('language') else ""
            stars_str = f" [Stars: {r.get('stargazers_count', 0)}]"
            context_lines.append(f"- {r['full_name']}{stars_str}{lang_str}: {r.get('description', '')} (Topics: {r.get('topics', [])})")
    ...
    system_prompt = (
        ...
        "2. 为每个赛道起一个精准、响亮的中文名称（例如“端侧多模态推理运行时”、“本地优先的知识库引擎”）。\n"
...
async def generate_weekly_comprehensive_insight(weekly_rising, trending_leaders, gems, tracks, user_bursts, return_model: bool = False):
    """
    ...
    context_sections = []
    if weekly_rising:
        context_sections.append("## [信号 1] 本周 7 日增长之王 (Weekly Velocity Leaders)")
        for r in weekly_rising[:15]:
            v7 = r.get('star_velocity_7d') or 0
            context_sections.append(f"- {r['full_name']}: {r.get('description', '')} (7天日均增量: +{int(v7)} stars/天, 累计: {r.get('stargazers_count', 0)} stars)")
    
    if trending_leaders:
        context_sections.append("\n## [信号 2] 本周 GitHub Trending 持续霸榜焦点 (Trending Leaders)")
        for t in trending_leaders[:10]:
            days = t.get('trending_days', 1)
            context_sections.append(f"- {t['full_name']}: {t.get('description', '')} (本周登榜: {days} 天, 累计: {t.get('stargazers_count', 0)} stars)")

    if gems:
        context_sections.append("\n## [信号 3] 图算法挖掘的隐藏宝藏 (PageRank Hidden Gems)")
        for g in gems[:15]:
            context_sections.append(f"- {g['full_name']}: {g.get('description', '')} (PR能量值: {g['pr_score']:.6f}, 仅 {g.get('stars', 0)} stars)")

    if tracks:
        context_sections.append("\n## [信号 4] 核心技术赛道全景 (Sector Clusters)")
        for t in tracks:
    ...
    if user_bursts:
        context_sections.append("\n## [信号 5] 核心开发者活跃度爆发 (Key Person Bursts)")
    ...
    system_prompt = (
        ...
        "我会为你提供本周 GitHub 关联网络挖掘出的全维度原始信号（含7天累计增长速度、趋势榜持续表现、图能量值、社区聚类和开发者异动）。\n"
        ...
        "2. **🔄 范式分化与技术迁移观察**：观察不同赛道之间的关联或竞争。例如：是否存在某种技术栈对旧生态的“大重构”？是否存在从单点工具向系统级方案的整合？\n"
...
async def generate_weekly_report_blocks(analyzer=None):
    """
    生成每周深度挖掘报告 (Weekly Deep Dive)。
    可复用外部已初始化的 GraphAnalyzer，彻底避免重复建图与图算法重复计算。
    """
    ...
    if analyzer is None:
        ...
        analyzer.build_network(limit_days=60)
    ...
    # 1. 准备核心数据 (引入周度 7 日增长冠军和 Trending 持续霸榜项目)
    from .velocity_calc import get_weekly_rising_stars, get_weekly_trending_leaders
    weekly_rising = get_weekly_rising_stars(limit=15)
    trending_leaders = get_weekly_trending_leaders(limit=10)
    user_bursts = user_analyzer.analyze_kp_activity_bursts()
    gems = analyzer.get_hidden_gems(top_n=15, max_stars=5000)
    ...
    for c_id, repo_ids in sorted_tracks:
        ...
        repos_data = db_manager.execute_query(
            f"SELECT full_name, description, topics, language, stargazers_count FROM repos WHERE id IN ({id_str})", 
            db_type="source"
        )
    ...
    # 获取全周综合影响力 Top 50 (基于 PageRank 能量结合 7 天活跃增量)
    top_50_repos = db_manager.execute_query(
        "SELECT full_name, description, stargazers_count, influence_score, star_velocity_7d "
        "FROM repos WHERE influence_score > 0 "
        "ORDER BY influence_score DESC, star_velocity_7d DESC, stargazers_count DESC LIMIT 50",
        db_type="source"
    )

    # 2. 并发调用 AI 解析引擎 (确保周报项目也有 zread/LLM 深度内容)
    all_repos_to_analyze = list(dict.fromkeys(
        [r['full_name'] for r in weekly_rising] + 
        [t['full_name'] for t in trending_leaders] +
        [g['full_name'] for g in gems] + 
    ...
    macro_report, macro_model = await generate_weekly_comprehensive_insight(
        weekly_rising, trending_leaders, gems, tracks_for_llm, user_bursts, return_model=True
    )
    ...
    # --- 第一板块: 🏆 本周 7 日增长之王 ---
    if weekly_rising:
        blocks.append({"object": "block", "type": "heading_2", "heading_2": {"rich_text": [{"type": "text", "text": {"content": "🏆 本周 7 日增长之王 (Weekly Velocity Top 15)"}, "annotations": {"color": "orange", "bold": True}}]}})
        
        for g in range(0, len(weekly_rising), 5):
            group_repos = weekly_rising[g:g+5]
            group_title = f"🚀 周度增长旗舰 #{g+1} - #{g+len(group_repos)}"
            group_blocks = []
            
            for i, repo in enumerate(group_repos, g + 1):
                fn = repo['full_name']
                summary = sanitize_ai_summary(ai_summaries.get(fn, repo.get('description') or "暂无解析。"))
                z_link = get_zread_link(fn)
                
                v_7d = repo.get('star_velocity_7d') or 0
                stars_total = repo.get('stargazers_count') or 0
                vel_str = f"📈 7日平均新增: `+{int(v_7d)} stars/天` | ⭐ 累计 Stars: `{stars_total}`"
    ...
    # --- 第二板块: 🔥 本周 GitHub Trending 连击榜 ---
    if trending_leaders:
        blocks.append({"object": "block", "type": "heading_2", "heading_2": {"rich_text": [{"type": "text", "text": {"content": "🔥 本周 GitHub Trending 持续霸榜项目"}, "annotations": {"color": "red", "bold": True}}]}})
        trend_group = []
        for i, t in enumerate(trending_leaders, 1):
            fn = t['full_name']
            summary = sanitize_ai_summary(ai_summaries.get(fn, t.get('description') or "暂无解析。"))
            z_link = get_zread_link(fn)
            days = t.get('trending_days', 1)
            v7 = t.get('star_velocity_7d') or 0
            vel_info = f" | 📈 7日新增: `+{int(v7)}/天`" if v7 > 0 else ""
            trend_group.append(create_callout_block(f"**{i}. {fn}** (本周登榜 `{days}` 天)", emoji="⚡", color="red_background"))
            trend_group.append({"object": "block", "type": "paragraph", "paragraph": {"rich_text": notion_client._parse_rich_text(f"[🔗 GitHub](https://github.com/{fn}) | [📖 zread.ai]({z_link})\n⭐ 累计 Stars: `{t.get('stargazers_count', 0)}`{vel_info}")}})
            trend_group.append({"object": "block", "type": "quote", "quote": {"rich_text": notion_client._parse_rich_text(summary)}})
            trend_group.append({"object": "block", "type": "divider", "divider": {}})
        blocks.append(create_toggle_block(f"📦 查看本周持续登榜的 {len(trending_leaders)} 个焦点项目", trend_group))

    # --- 第三板块: 🕵️‍♂️ 能量中心：PageRank 隐藏宝藏 (扫地僧) ---
    blocks.append({"object": "block", "type": "heading_2", "heading_2": {"rich_text": [{"type": "text", "text": {"content": "🕵️‍♂️ 能量中心：PageRank 隐藏宝藏 (扫地僧 Top 15)"}, "annotations": {"color": "purple", "bold": True}}]}})
    gem_blocks = []
    for i, gem in enumerate(gems[:15], 1):
        fn = gem['full_name']
        summary = sanitize_ai_summary(ai_summaries.get(fn, gem.get('description') or "No description"))
        gem_blocks.append(create_callout_block(f"**{i}. {fn}**", emoji="💎", color="purple_background"))
    ...
    # --- 第四板块: 🚦 技术赛道全景 ---
    blocks.append({"object": "block", "type": "heading_2", "heading_2": {"rich_text": [{"type": "text", "text": {"content": "🚦 技术赛道全景 (Sector Analysis)"}, "annotations": {"color": "blue", "bold": True}}]}})
    ...
    # --- 第五板块: 🌟 本周综合影响力之星 Top 50 ---
    blocks.append({"object": "block", "type": "heading_2", "heading_2": {"rich_text": [{"type": "text", "text": {"content": "🌟 本周综合影响力之星 Top 50"}, "annotations": {"color": "gray", "bold": True}}]}})
    group_size = 10
    for g in range(0, len(top_50_repos), group_size):
        ...
        for i, repo in enumerate(group_repos, g + 1):
            fn = repo['full_name']
            z_link = get_zread_link(fn)
            summary = sanitize_ai_summary(ai_summaries.get(fn, repo.get('description') or "No description"))
            group_blocks.append({
...
def generate_weekly_report(analyzer=None):
    blocks = asyncio.run(generate_weekly_report_blocks(analyzer=analyzer))
